...
//         System.out.println(o.changeOrder("ORD0000001"));
    ...
    public List<OrderDetail> getOrderDetail(String id) {
        ...
        try {
            String sql = "select ord_id, f.per_name, quantity, price from OrderDetail o join Perfume f on o.per_id= f.per_id where ord_id = ?";
            PreparedStatement ps = connection.prepareStatement(sql);
            ps.setString(1, id);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
    ...
    private void addOrderDetails(String orderId, List<Cart> carts) {
        ...
        try {
            for (Cart cart : carts) {
                ...
                ps = connection.prepareStatement(sql);
                ps.setString(1, id);
                ps.setString(2, cart.getPerfume().getId());
                ps.setInt(3, cart.getQuantity());
                ps.setDouble(4, cart.getPerfume().getRealPrice());
                ps.executeUpdate();
            }
