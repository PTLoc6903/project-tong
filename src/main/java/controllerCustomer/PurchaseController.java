...
import daoGuestCustomer.CustomerDAO;
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!carts.isEmpty()) {
            ...
            for (int i = 0; i < carts.size(); i++) {
                sum += (double) carts.get(i).getQuantity() * carts.get(i).getPerfume().getRealPrice();
            }
            request.setAttribute("customer", customer);
            request.setAttribute("carts", carts);
            request.setAttribute("total", (double) Math.round(sum * 100) / 100);
            request.getRequestDispatcher("purchaseCustomer.jsp").forward(request, response);
        } else {
