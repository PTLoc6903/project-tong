...
import daoGuestCustomer.CartDAO;
...
protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    String perfumeID = request.getParameter("perfumeID");  // Đảm bảo đúng tên tham số
    CartDAO c = new CartDAO();
    
    if (status.equals("true")) {
        c.updateQuantity(quantity + 1, phone, perfumeID);
    } else if (status.equals("false")) {
        if (quantity > 1) {
            c.updateQuantity(quantity - 1, phone, perfumeID);
        }
    } else {
        c.deleteCart(phone, perfumeID);
    }

    List<Cart> carts = c.searchByPhone(phone);
    PrintWriter out = response.getWriter();
    double totalPricePerfume = 0;
    double sum = 0;
    String s = "  <div class=\"header\">\n"
            ...
            + "                  <tbody>";
    for (Cart cart : carts) {
        totalPricePerfume = cart.getPerfume().getRealPrice() * cart.getQuantity();
        totalPricePerfume = (double) Math.round(totalPricePerfume * 100) / 100;
        sum += totalPricePerfume;
        s += "    <tr>\n"
                + "                                    <td class=\"perfume-info\">\n"
                + "                                        <img src=\"" + cart.getPerfume().getImg() + "\" alt=\"\">\n"
                + "                                    </td>\n"
                + "                                    <td style=\"font-size: 23px;\"> <a href=\"#\">" + cart.getPerfume().getName() + "</a> </td>\n"
                + "                                    <td>" + totalPricePerfume + "$</td>\n"
                + "                                    <td>\n"
                ...
                + "                                    <td>\n"
                + "                                <!-- View detail -->\n"
                + "                                        <a class=\"btn\" href=\"perfumeDetail?id=" + cart.getPerfume().getId() + "\">\n"
